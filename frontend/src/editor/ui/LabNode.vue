<template>
  <div class="node" :class="{ selected: data.selected }" :style="nodeStyle">
    ...
    <div
      ...
      :key="key"
      :ref="el => onRef((el as HTMLElement), key, control, 'control')"
    ></div>
    <div class="interface">
      <div class="inputs">
        <!-- INPUT -->
        <div class="input" v-for="[key, input] in inputs" :key="key">
          <div
            class="input-socket"
            :ref="(el) => onRef((el as HTMLElement), key, input, 'input')"
          ></div>
      ...
      <div class="outputs">
        ...
        <div class="output" v-for="[key, output] in outputs" :key="key">
          ...
          <div
            class="output-socket"
            :ref="(el) => onRef((el as HTMLElement), key, output, 'output')"
          ></div>
        </div>
      </div>
    </div>
...
const sortByIndex = (
  entries: [string, labelWithIndexType][]
): [string, labelWithIndexType][] => {
...
const nodeStyle = computed(() => {
  const nodeHeight = Number.isFinite(props.data.height)
    ? `${props.data.height}px`
    : "";
  const nodeWidth = Number.isFinite(props.data.width)
    ? `${props.data.width}px`
    : "";
  return {
    height: nodeHeight,
...
.node {
  ...
  &.selected {
    box-shadow: 0 0 4px 4px rgb(90 175 250 / 28%);
    background-color: rgba(52, 76, 97, 0.836);
  }
